...
bool VolumetricPass::Initialize(ID3D11Device* device) {
    ...
    if (!m_volumetricShader.LoadVertexShader(device, L"shaders/volumetric.hlsl", "VS", fsLayout)) return false;
    if (!m_volumetricShader.LoadPixelShader(device, L"shaders/volumetric.hlsl", "PS")) return false;

    // Initialize constant buffer
    if (!m_volumetricBuffer.Initialize(device)) return false;

    // Set default parameters
    m_params.params = { Config::Volumetric::DEFAULT_STEP_COUNT,
...
void VolumetricPass::Execute(ID3D11DeviceContext* context,
