...
bool ShadowPass::Initialize(ID3D11Device* device) {
    ...
    if (!m_shadowShader.LoadVertexShader(device, L"shaders/shadow.hlsl", "VS", layout)) return false;
    if (!m_shadowShader.LoadPixelShader(device, L"shaders/shadow.hlsl", "PS")) return false;
...
void ShadowPass::Execute(ID3D11DeviceContext* context,
    ...
    DirectX::XMVECTOR lPos = DirectX::XMVectorSet(
        spotData.posRange.x, spotData.posRange.y, spotData.posRange.z, 1.0f);
    DirectX::XMVECTOR lDir = DirectX::XMVector3Normalize(DirectX::XMVectorSet(
        spotData.dirAngle.x, spotData.dirAngle.y, spotData.dirAngle.z, 0.0f));
    ...
    DirectX::XMMATRIX lProj = DirectX::XMMatrixPerspectiveFovLH(
        DirectX::XM_PIDIV2, 1.0f, 0.1f, spotData.posRange.w);
